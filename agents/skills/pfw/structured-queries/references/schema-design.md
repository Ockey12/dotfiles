...
Introduce the CasePaths library and enable the `CasePaths` package trait to add support for enum column groupings with a column per case to simulate single table inheritance:
...
`@Selection` will automatically generate a `CasePathable` conformance for `Media`. No need to apply `@CasePathable`.
